...
def servers_edit():
    if request.method == 'GET':
        ...
        if(check_permission(session['username']) != 10 and session['username'] not in get_server_by_name(srvname)['owner']):
        ...
        if(check_permission(session['username']) != 10 and session['username'] not in get_server_by_name(srvname)['owner']):
            return ("Permission denied", 403)
        if(is_valid_host(newhost) == False):
            return render_template(
                'servers_edit.html',
                siteinfo=siteinfo,
                server=get_server_by_name(srvname),
...
def changepass():
    ...
    if(request.method == 'GET'):
        ...
        if(request.form['old_password'] == "" or request.form['new_password'] == "" or request.form['retype_password'] == ""):
            # Username or password is empty
        ...
        if(is_valid_password(request.form['new_password']) == False):
            # Passwords isn't valid
            return render_template('changepass.html', siteinfo=siteinfo, message='密码无效：长度至少为8为，不能包含除数字、字母、.和#以外的字符。')
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port='1356', debug=False)
